...
export default function ChallengeBoard() {
  ...
  useEffect(() => {
    ...
    async function loadChallenges() {
      ...
      try {
        const doc = await getDocument<{ completedIds: string[] }>(
  ...
  return (
    <section aria-labelledby="challenges-heading" className="space-y-6">
      {/* Progress Card */}
      <div className="flex flex-col gap-8 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="flex flex-col items-center justify-between gap-8 md:flex-row">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 id="challenges-heading" className="mb-1 text-xl font-bold text-slate-900">
                Your Progress
              </h2>
          ...
          <div className="flex items-center gap-8 md:gap-12">
            ...
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 text-[#22c55e]">
              ...
              <div>
                <p className="text-lg leading-none font-black text-slate-900">{completedCount}</p>
                <p className="mt-0.5 text-[10px] font-bold text-slate-500">Completed</p>
              </div>
            </div>

            {/* CO2e Saved */}
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#eaf6ec] text-[#059669]">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <div>
                <p className="text-lg leading-none font-black text-slate-900">
                  {totalSaving.toFixed(1)}
                </p>
                <p className="mt-0.5 text-[10px] font-bold text-slate-500">kg CO₂e Saved</p>
              </div>
            </div>

            {/* Total */}
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-amber-400">
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>
              <div>
                <p className="text-lg leading-none font-black text-slate-900">{totalCount}</p>
                <p className="mt-0.5 text-[10px] font-bold text-slate-500">Total Challenges</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="h-2.5 overflow-hidden rounded-full bg-slate-100">
            <div
              className="h-full rounded-full bg-[#22c55e] transition-all duration-1000 ease-out"
      ...
      <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
        <div className="flex flex-wrap items-center gap-2">
          <button className="flex items-center gap-2 rounded-lg border border-[#eaf6ec] bg-[#eaf6ec] px-4 py-2 text-xs font-bold text-[#059669]">
          ...
          <button className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
        ...
        <button className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50">
          Sort by: Recommended
          <svg
            className="h-3 w-3 text-slate-400"
            fill="none"
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {challenges.map((challenge) => (
          <ChallengeCard key={challenge.id} challenge={challenge} onToggle={handleToggle} />
        ))}
      </div>
    </section>
  );
